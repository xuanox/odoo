odoo.define('regulatory_management.website_regulatory_booking_system', function (require) {
'use strict';
    ...
    $(document).on('click',"#submit_button",function() {
        ...
        var phone = $("#phone").val();
        var email = $("#email").val();
        var service = $('.check_box_regulatory:checkbox:checked');
        var chair = $( "#chair" ).val();
        var list_service = [];
        ...
        if (name == "" || date == "" || time == "" || phone == "" || email == "" || list_service.length == 0){
            ...
            if (isNaN(time_left_char) || isNaN(time_right_char) || time_separator != ":"){
                ...
                if ((time_left < 24) && (time_right < 60) && (time_left >= 0) && (time_right >= 0)){
                    ...
                    $.ajax({
                        url: "/page/regulatory_details",
                        type: "POST",
                        dataType: "json",
                        data: booking_record,
                        type: 'POST',
                        success: function( data ) {
                            window.location.href = "/page/regulatory_management.regulatory_booking_thank_you";
                            },
    ...
    $(document).on('click',"#check_button",function() {
        ...
        if (check_date != ""){
            ajax.jsonRpc("/page/regulatory_check_date", 'call', {'check_date':check_date })
