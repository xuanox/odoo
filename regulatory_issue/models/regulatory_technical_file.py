...
class RegulatoryTechnicalFileCreation(models.Model):
    ...
    sales_team_id = fields.Many2one('crm.team', string='Sales Team', track_visibility='onchange', default=lambda self: self.env['crm.team'].sudo()._get_default_team_id(users_id=self.env.uid),index=True)
