...
#############################Edicion Ceila##################################################
    @api.one
    @api.depends('line_ids', 'line_ids.amount, balance_end')
    def _get_total(self):
       if x_studio_en_transito == True:
           self.x_studio_en_transito = sum([line.amount for line in self.line_ids])
        else 
            x_studio_en_transito = self.balance_end 

 x_studio_en_transito = fields.Float(compute='_get_total', string="")
