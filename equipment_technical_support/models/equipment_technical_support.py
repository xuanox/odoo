...
class equipment_equipment(models.Model):
    ...
    def _compute_next_maintenance(self):
        ...
        for equipment in self.filtered(lambda x: x.period > 0):
            next_maintenance_todo = self.env['technical_support.order'].search([
                ('equipment_id', '=', equipment.id),
                ('maintenance_type', '=', 'pm'),
                ('state', '=', 'done'),
                ('close_date', '=', False)], order="request_date asc", limit=1)
            last_maintenance_done = self.env['technical_support.order'].search([
                ('equipment_id', '=', equipment.id),
                ('maintenance_type', '=', 'pm'),
                ('state', '=', 'done'),
    ...
    def _create_new_request(self, date):
        self.ensure_one()
        self.env['technical_support.order'].create({
    ...
    def _cron_generate_requests(self):
        ...
        for equipment in self.search([('period', '>', 0)]):
            next_requests = self.env['technical_support.order'].search([('state', '=', 'done'),
                                                    ('equipment_id', '=', equipment.id),
                                                    ('maintenance_type', '=', 'pm'),
