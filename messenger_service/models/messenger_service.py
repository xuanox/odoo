...
class MessengerServiceStage(models.Model):
    _name = 'messenger.service.stage'
    _description = 'Messenger Service Stage'
    _order = 'sequence, id'

    name = fields.Char('Name', required=True, translate=True)
    sequence = fields.Integer('Sequence', default=20)
    fold = fields.Boolean('Folded in Messenger Service Pipe')
    done = fields.Boolean('Request Done')

class MessengerServiceType(models.Model):
    _name = 'messenger.service.type'
    _description = ' Messenger Service Type'
    _order = 'id'

    name = fields.Char(string="Type", required=True, translate=True)

class MessengerService(models.Model):
    _name = 'messenger.service'
    _description = 'Messenger Service'
    _order = 'id'
    ...
    type_id = fields.Many2one('messenger.service.type', u'Type')
    stage_id = fields.Many2one('messenger.service.stage', string='Stage')
