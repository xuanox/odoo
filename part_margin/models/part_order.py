...
class PartLine(models.Model):
    ...
    def _compute_margin(self, part_id, product_id, product_uom_id):
        frm_cur = self.env.user.company_id.currency_id
        to_cur = part_id.pricelist_id.currency_id
        purchase_price = product_id.standard_price
        if product_uom_id != product_id.uom_id:
            purchase_price = product_id.uom_id._compute_price(purchase_price, product_uom_id)
        price = frm_cur._convert(
            purchase_price, to_cur, part_id.company_id, part_id.date_order or fields.Date.today(), round=False)
        return price
    ...
    def _get_purchase_price(self, pricelist, product, product_uom, date):
        ...
        price = frm_cur._convert(
            ...
            self.part_id.company_id or self.env.user.company_id,
            date or fields.Date.today(), round=False)
        return {'purchase_price': price}

    @api.onchange('product_id', 'product_uom')
    def product_id_change_margin(self):
        if not self.part_id.pricelist_id or not self.product_id or not self.product_uom:
            return
    ...
    def _product_margin(self):
        for line in self:
            currency = line.part_id.pricelist_id.currency_id
