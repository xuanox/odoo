...
class technical_supportWorkOrder(models.Model):
    _name = 'technical_support.workorder'
    ...
    company_id = fields.Many2one(
        'res.company', 'Company',
        default=lambda self: self.env['res.company']._company_default_get('technical_support.workorder'),
        required=True)
    mo_ids = fields.One2many('technical_support.order', 'wo_id', 'Maintenance Order')

    @api.model
    def create(self, values):
        if not values.get('name', False) or values['name'] == _('New'):
            values['name'] = self.env['ir.sequence'].next_by_code('technical_support.workorder') or _('New')
        return super(technical_supportWorkOrder, self).create(values)
